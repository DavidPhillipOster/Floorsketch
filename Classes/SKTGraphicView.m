...
NSString *const SKTGraphicViewGraphicsBindingName = @"graphics";
NSString *const SKTGraphicViewSelectionIndexesBindingName = @"selectionIndexes";
NSString *const SKTGraphicViewGridBindingName = @"grid";

// The values that are used as contexts by this class' invocations of KVO observer registration methods. When an object like this one receives an -observeValueForKeyPath:ofObject:change:context: message it has to figure out why it's getting the message. It could distinguish based on the observed object and key path, but that's not perfectly safe, because code in the superclass might be observing the same thing for a different reason, and there's a danger of intercepting observer notifications that are meant for superclass code. The way to make sure that doesn't happen is to use a context, and make sure it's unlikely to be used as a context by superclass or subclass code. Strings like these whose pointers are not available to other compiled modules are pretty unlikely to be used by superclass or subclass code. In practice this is not a common problem, especially in a simple application like FloorSketch, but you should know how to do things like this the perfect way even if you decide it's not worth the hassle in your application.
static char *const SKTGraphicViewGraphicsObservationContext = "com.turbozen.SKTGraphicView.graphics";
static char *const SKTGraphicViewIndividualGraphicObservationContext = "com.turbozen.SKTGraphicView.individualGraphic";
static char *const SKTGraphicViewSelectionIndexesObservationContext = "com.turbozen.SKTGraphicView.selectionIndexes";
static char *const SKTGraphicViewAnyGridPropertyObservationContext = "com.turbozen.SKTGraphicView.anyGridProperty";
...
static NSString *const SKTGraphicViewPasteboardType = @"FloorSketch pasteboard type";
...
@interface SKTGraphicView()<SKTHasHandles> {
  ...
  // The grid that is drawn in the view and used to constrain graphics as they're created and moved. In FloorSketch this is just a cache of a value that canonically lives in the SKTWindowController to which this view's grid property is bound (see SKTWindowController's comments for an explanation of why the grid lives there).
  SKTGrid *_grid;
  ...
  if (context == SKTGraphicViewGraphicsObservationContext) {
    ...
    if (![oldGraphics isEqual:[NSNull null]]) {
      ...
      for (NSUInteger index = 0; index < graphicCount; index++) {
        [self setNeedsDisplayInRect:[self handleDrawingBoundsOfGraphic:oldGraphics[index]]];
      }
    ...
    if ([keyPath isEqualToString:SKTGraphicDrawingBoundsKey]) {
      ...
      oldGraphicDrawingBounds = [self insetForHandlesRect:oldGraphicDrawingBounds];
      [self setNeedsDisplayInRect:oldGraphicDrawingBounds];
      NSRect newGraphicDrawingBounds = [change[NSKeyValueChangeNewKey] rectValue];
      newGraphicDrawingBounds = [self insetForHandlesRect:newGraphicDrawingBounds];
    ...
    if (![oldSelectionIndexes isEqual:[NSNull null]] && ![newSelectionIndexes isEqual:[NSNull null]]) {
      for (NSUInteger oldSelectionIndex = [oldSelectionIndexes firstIndex]; oldSelectionIndex != NSNotFound; oldSelectionIndex = [oldSelectionIndexes indexGreaterThanIndex:oldSelectionIndex]) {
        if (![newSelectionIndexes containsIndex:oldSelectionIndex]) {
          ...
          [self setNeedsDisplayInRect:[self handleDrawingBoundsOfGraphic:deselectedGraphic]];
        }
      }
      for (NSUInteger newSelectionIndex = [newSelectionIndexes firstIndex]; newSelectionIndex != NSNotFound; newSelectionIndex = [newSelectionIndexes indexGreaterThanIndex:newSelectionIndex]) {
        if (![oldSelectionIndexes containsIndex:newSelectionIndex]) {
          ...
          [self setNeedsDisplayInRect:[self handleDrawingBoundsOfGraphic:selectedGraphic]];
        }
      }
    } else {
      [self setNeedsDisplay:YES];
    ...
    // Either a new grid is to be used (this only happens once in FloorSketch) or one of the properties of the grid has changed. Regardless, redraw everything.
    [self setNeedsDisplay:YES];
...
// Insets by a negative amount, but "out-set" isn't a word.
- (NSRect)insetForHandlesRect:(NSRect)r {
  if ( ! CGRectIsEmpty(r)) {
    CGFloat outset = [self handleWidth] / 2.0f;
    CGFloat inset = 0.0f - outset;
    r = NSInsetRect(r, inset, inset);
  }
  return r;
}
...
- (NSRect)handleDrawingBoundsOfGraphic:(SKTGraphic *)graphic {
  return [self insetForHandlesRect:[graphic drawingBounds]];
}

- (NSRect)handleDrawingBoundsOfGraphics:(NSArray *)graphics {
  ...
  if (graphicCount > 0) {
    drawingBounds = [self handleDrawingBoundsOfGraphic:graphics[0]];
    for (NSUInteger index = 1; index < graphicCount; index++) {
      drawingBounds = NSUnionRect(drawingBounds, [self handleDrawingBoundsOfGraphic:graphics[index]]);
    }
  }
...
- (void)drawRect:(NSRect)rect {
  ...
  [(id<SKTGraphicsOwner>)self drawGraphics:self.graphics view:self rect:rect];
...
- (BOOL)isBeingCreateOrEdited:(SKTGraphic *)graphic {
  return nil != graphic && (graphic==_creatingGraphic || graphic==_editingGraphic);
}

// The GraphicView overrides these GraphicOwner methods.
- (BOOL)isHidingHandles {
  return _isHidingHandles;
}
...
- (void)startEditingGraphic:(SKTGraphic *)graphic {
  ...
  if (_editingView) {
    ...
    // Give the graphic being edited a chance to draw one more time. In FloorSketch, SKTText draws a focus ring.
    [self setNeedsDisplayInRect:[self handleDrawingBoundsOfGraphic:_editingGraphic]];
  }
}
...
- (void)stopEditing {
  ...
  if (_editingView) {
    ...
    BOOL makeSelfFirstResponder = ([[self window] firstResponder] == _editingView);
    [_editingView removeFromSuperview];
    if (makeSelfFirstResponder) {
      [[self window] makeFirstResponder:self];
    }

    // If the editing view added a ruler accessory view then remove it because it's not applicable anymore, and get rid of the blank space in the ruler that would otherwise result. In FloorSketch the NSTextViews created by SKTTexts leave horizontal ruler accessory views.
...
- (SKTGraphic *)graphicUnderPoint:(NSPoint)point index:(NSUInteger *)outIndex isSelected:(BOOL *)outIsSelected handle:(NSInteger *)outHandle {
  ...
  for (NSUInteger index = 0; index < graphicCount; index++) {
    ...
    if (NSPointInRect(point, [self handleDrawingBoundsOfGraphic:graphic])) {
      ...
      if (graphicIsSelected) {
        NSInteger handle = [graphic handleUnderPoint:point inView:self];
...
- (void)moveSelectedGraphicsWithEvent:(NSEvent *)event {
  ...
  NSRect selBounds = [self handleDrawingBoundsOfGraphics:selGraphics];
  ...
  if (isMoving) {
    _isHidingHandles = NO;
    [self setNeedsDisplayInRect:[self handleDrawingBoundsOfGraphics:selGraphics]];
    if (didMove) {
...
- (void)addPtsToGraphic:(SKTPoly *)graphic withEvent:(NSEvent *)event {
  BOOL echoToRulers = [[self enclosingScrollView] rulersVisible];
  if (echoToRulers) {
  ...
  [graphic insertPt:handleLocation atIndex:0];

  BOOL isDone = NO;
  NSInteger pointCount = 1;
  NSTimeInterval lastEventTimeStamp  = 0;
  while ( ! isDone) {
    event = [[self window] nextEventMatchingMask:(NSLeftMouseDraggedMask | NSLeftMouseUpMask | NSMouseMovedMask | NSLeftMouseDownMask | NSKeyDownMask)];
    if ([event type] == NSEventTypeKeyDown) {
      isDone = YES;
    } else if ([event type] == NSLeftMouseDown && ([event timestamp] - lastEventTimeStamp < [NSEvent doubleClickInterval])) {
      isDone = YES;
    } else {
      ...
      if (pointCount < graphic.countOfPt) {
        [graphic removePtAtIndex:pointCount];
      }
      [graphic insertPt:handleLocation atIndex:pointCount];
...
- (NSIndexSet *)indexesOfGraphicsIntersectingRect:(NSRect)rect {
  ...
  for (NSUInteger index = 0; index < graphicCount; index++) {
    SKTGraphic *graphic = graphics[index];
    if (NSIntersectsRect(rect, [self handleDrawingBoundsOfGraphic:graphic])) {
...
- (void)createGraphicOfClass:(Class)graphicClass withEvent:(NSEvent *)event {
  ...
  if ([_creatingGraphic respondsToSelector:@selector(addPt:)]) {
    [self addPtsToGraphic:(SKTPoly *)_creatingGraphic withEvent:event];
  } else {
...
- (void)selectAndTrackMouseWithEvent:(NSEvent *)event {
  ...
  BOOL modifyingExistingSelection = (0 != ([event modifierFlags] & (NSCommandKeyMask | NSShiftKeyMask)));
...
- (void)mouseDown:(NSEvent *)event {
  ...
  // If a graphic has been being edited (in FloorSketch SKTTexts are the only ones that are "editable" in this sense) then end editing.
  [self stopEditing];
...
- (void)invalidateHandlesOfGraphics:(NSArray *)graphics {
  ...
  for (i = 0; i < c; i++) {
    [self setNeedsDisplayInRect:[self handleDrawingBoundsOfGraphic:graphics[i]]];
  }
}
...
- (void)unhideHandlesForTimer:(NSTimer *)timer {
  ...
  [self setNeedsDisplayInRect:[self handleDrawingBoundsOfGraphics:[self selectedGraphics]]];
}
...
- (BOOL)validateMenuItem:(NSMenuItem *)item {
  ...
  if (action == @selector(makeNaturalSize:)) {
    ...
    // These only apply if there is a selection where at least one is unlocked.
    return 0 < [[self selectedGraphics] countByFilteringWithInverseSelector:@selector(locked)];
    ...
    // These only apply to multiple selection where at least one is unlocked.
    return 1 < [(selGraphics = [self selectedGraphics]) count] &&
        0 < [selGraphics countByFilteringWithInverseSelector:@selector(locked)];
    ...
    [item setTitle:([[self enclosingScrollView] rulersVisible] ? NSLocalizedStringFromTable(@"Hide Ruler", @"MenuItems", @"A main menu item title.") : NSLocalizedStringFromTable(@"Show Ruler", @"MenuItems", @"A main menu item title."))];
    return YES;
  } else if (action == @selector(group:)) {
    return [self canGroup];
  } else if (action == @selector(ungroup:)) {
    return [self canUngroup];
  } else if (action == @selector(lock:)) {
    return 0 < [[self selectedGraphics] countByFilteringWithInverseSelector:@selector(locked)];  // some are not locked
  } else if (action == @selector(unlock:)) {
    return 0 < [[self selectedGraphics] countByFilteringWithSelector:@selector(locked)]; // some are locked
  } else if (action == @selector(openPolygon:)) {
    return 0 < [[self selectedGraphics] countByFilteringWithSelector:@selector(canOpenPolygon)];
  } else if (action == @selector(closePolygon:)) {
    return 0 < [[self selectedGraphics] countByFilteringWithSelector:@selector(canClosePolygon)];
  }else {
    return YES;
  }
}
...
// Only allow grouping if more than one item is selected and all are locked or all are unlocked
- (BOOL)canGroup {
  NSArray *selected = [self selectedGraphics];
  NSInteger selectedCount = [selected count];
  return 1 < selectedCount && selectedCount == [selected countByFilteringWithInverseSelector:@selector(locked)];
}

- (IBAction)group:(id)sender {
  NSArray *selection = [self selectedGraphics];
  NSMutableArray *graphics = [self mutableGraphics];
  NSUInteger firstIndex = NSNotFound;
  BOOL isLocked = [selection.firstObject locked];
  for (SKTGraphic *item in selection) {
    NSUInteger index = [graphics indexOfObject:item];
    if (NSNotFound != index) {
      [graphics removeObjectAtIndex:index];
      if (firstIndex == NSNotFound || index < firstIndex) {
        firstIndex = index;
      }
    }
  }
  SKTGroup *group = [[SKTGroup alloc] init];
  group.graphics = [selection mutableCopy];
  [graphics insertObject:group atIndex:firstIndex];
  [group setLocked:isLocked];
  NSIndexSet *indexes = [NSIndexSet indexSetWithIndex:firstIndex];
  [self changeSelectionIndexes:indexes];
  [[self undoManager] setActionName:NSLocalizedStringFromTable(@"Group", @"UndoStrings", @"Action name for make selected a group.")];
}

- (BOOL)canUngroup {
  return 0 < [[self selectedGraphics] countByFilteringWithClass:[SKTGroup class]];  // some are grouped.
}

- (IBAction)ungroup:(id)sender {
  NSArray *groups = [[self selectedGraphics] arrayByFilteringWithClass:[SKTGroup class]];
  NSMutableArray *graphics = [self mutableGraphics];
  NSMutableArray *allChildren = [NSMutableArray array];
  for (SKTGroup *group in groups) {
    BOOL isLocked = [group locked];
    NSArray *children = [group graphics];
    [children makeObjectsPerformSelector:@selector(setLockedValue:) withObject:@(isLocked)];
    [allChildren addObject:children];
    NSUInteger index = [graphics indexOfObject:group];
    if (NSNotFound != index) {
      [graphics removeObjectAtIndex:index];
      [graphics insertObjects:children atIndexes:[NSIndexSet indexSetWithIndexesInRange:NSMakeRange(index, children.count)]];
    }
  }
  NSMutableIndexSet *indexes = [NSMutableIndexSet indexSet];
  for (NSArray *children in allChildren) {
    NSUInteger index = [[self graphics] indexOfObject:children.firstObject];
    if (NSNotFound != index) {
      [indexes addIndexes:[NSIndexSet indexSetWithIndexesInRange:NSMakeRange(index, children.count)]];
    }
  }
  [self changeSelectionIndexes:indexes];
  [[self undoManager] setActionName:NSLocalizedStringFromTable(@"Ungroup", @"UndoStrings", @"Action name for ungrouping a group.")];
}

- (IBAction)lock:(id)sender {
  [[self selectedGraphics] makeObjectsPerformSelector:@selector(setLockedValue:) withObject:@YES];
}

- (IBAction)unlock:(id)sender {
  [[self selectedGraphics] makeObjectsPerformSelector:@selector(setLockedValue:) withObject:@NO];
}

- (BOOL)canOpenPolygon {
  NSArray *selected = [self selectedGraphics];
  return 1 < [selected countByFilteringWithSelector:@selector(canOpenPolygon)];
}

- (IBAction)openPolygon:(id)sender {
  NSArray *selected = [self selectedGraphics];
  NSMutableArray *graphics = [self mutableGraphics];
  NSMutableIndexSet *indexes = [NSMutableIndexSet indexSet];
  for (SKTGraphic *graphic in selected) {
    if ([graphic canOpenPolygon]) {
      NSUInteger index = [graphics indexOfObject:graphic];
      if (NSNotFound != index) {
        SKTGraphic *newGraphic = [graphic graphicByOpening];
        [graphics setObject:newGraphic atIndexedSubscript:index];
        [indexes addIndex:index];
      }
    }
  }
  [self changeSelectionIndexes:indexes];
  [[self undoManager] setActionName:NSLocalizedStringFromTable(@"Open Polygon", @"UndoStrings", @"Action name for opening a polygon.")];
}

- (BOOL)canClosePolygon {
  NSArray *selected = [self selectedGraphics];
  return 1 < [selected countByFilteringWithSelector:@selector(canClosePolygon)];
}

- (IBAction)closePolygon:(id)sender {
  NSArray *selected = [self selectedGraphics];
  NSMutableArray *graphics = [self mutableGraphics];
  NSMutableIndexSet *indexes = [NSMutableIndexSet indexSet];
  for (SKTGraphic *graphic in selected) {
    if ([graphic canClosePolygon]) {
      NSUInteger index = [graphics indexOfObject:graphic];
      if (NSNotFound != index) {
        SKTGraphic *newGraphic = [graphic graphicByClosing];
        [graphics setObject:newGraphic atIndexedSubscript:index];
        [indexes addIndex:index];
      }
    }
  ...
  [self changeSelectionIndexes:indexes];
  [[self undoManager] setActionName:NSLocalizedStringFromTable(@"Close Polygon", @"UndoStrings", @"Action name for closig a polygon.")];
}
...
- (BOOL)rulersVisible {
  ...
  return [enclosingScrollView rulersVisible];
}

- (void)setRulersVisible:(BOOL)rulersVisible {
  NSScrollView *enclosingScrollView = [self enclosingScrollView];
  [enclosingScrollView setRulersVisible:rulersVisible];
}
