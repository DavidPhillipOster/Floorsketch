...
static char *const SKTWindowControllerCanvasSizeObservationContext = "com.turbozen.SKTWindowController.canvasSize";

@interface SKTWindowController() {
  ...
  NSDictionary *_propertiesWhileOpening;
}
@end
...
- (void)selectedToolDidChange:(NSNotification *)notification {
  ...
  SKTToolPaletteController *tools = [SKTToolPaletteController sharedToolPaletteController];
  Class theClass = [tools currentGraphicClass];
  NSCursor *theCursor = nil;
  if (theClass) {
    theCursor = [theClass creationCursor];
  }
  if (nil == theCursor) {
    theCursor = [tools currentSelectionCursor];
  }
  [[_graphicView enclosingScrollView] setDocumentCursor:theCursor];
}

#pragma mark - Override NSWindowController Methods


- (void)setDocument:(SKTDocument *)document {
  ...
  _propertiesWhileOpening = document.propertiesWhileOpening;
  document.propertiesWhileOpening = nil;
...
- (void)windowDidLoad {
  ...
  // Finish restoring the document state now that we have a window to write into.
  [self setRulersVisible:nil != _propertiesWhileOpening[SKTDocumentVisibleRulerKey]];
  double zoomFactor = [(NSNumber *)_propertiesWhileOpening[SKTDocumentScaleKey] doubleValue];
  if (0 < zoomFactor && zoomFactor < 100) {
    [self setZoomFactor:zoomFactor];
  }
  double spacing = [(NSNumber *)_propertiesWhileOpening[SKTDocumentGridSpacingKey] doubleValue];
  if (0.0 < spacing) {
    [_grid setSpacing:spacing];
  }
  NSColor *color = [NSColor colorWithArchiveData:_propertiesWhileOpening[SKTDocumentGridColorKey]];
  if (color) {
    [_grid setColor:color];
  }
  if (_propertiesWhileOpening[SKTDocumentGridAlwaysShownKey]) {
    [_grid setAlwaysShown:YES];
  }
  if (_propertiesWhileOpening[SKTDocumentGridConstrainingKey]) {
    [_grid setConstraining:YES];
  }

  _propertiesWhileOpening = nil;
...
- (BOOL)rulersVisible {
  return [_graphicView rulersVisible];
}

- (void)setRulersVisible:(BOOL)rulersVisible {
  [_graphicView setRulersVisible:rulersVisible];
}
