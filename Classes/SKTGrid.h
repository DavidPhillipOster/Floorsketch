...
extern NSString *const SKTGridAnyKey;
...
/* This class is KVC and KVO compliant for these keys:
 ...
 In FloorSketch various properties of the controls of the grid inspector are bound to the properties (all except for the "any" property) of the grid belonging to the window controller of the main window. Each SKTGraphicView observes the "any" property of the grid to which its bound so it knows when the grid needs drawing.

 */

// Simple accessors.
@property (NS_NONATOMIC_IOSONLY, getter = isAlwaysShown) BOOL alwaysShown;
@property (NS_NONATOMIC_IOSONLY, getter = isConstraining) BOOL constraining;
@property (NS_NONATOMIC_IOSONLY, getter = isUsable, readonly) BOOL usable;
...
@property (NS_NONATOMIC_IOSONLY, readonly) BOOL canAlign;
